<?php
	...
	Class Model {
		...
		public function orgStructureSignIn($uname, $pword) {
			$query = "SELECT id, password FROM org_structure WHERE email = ? LIMIT 1";
			if($stmt = $this->conn->prepare($query)) {
				$stmt->bind_param("s", $uname);
				$stmt->execute();
		...
		public function orgStaffSignIn($uname, $pword) {
			$query = "SELECT id, password FROM org_structure WHERE email = ? AND position = '0' LIMIT 1";
			if($stmt = $this->conn->prepare($query)) {
				$stmt->bind_param("s", $uname);
				$stmt->execute();
				$stmt->bind_result($id, $hashed_pass);
		...
		public function updateResident($occupation, $birth_date, $birth_place, $gender, $civil_status, $contact_number, $email , $address3, $resident_since, $id) {
			$query = "UPDATE residents SET occupation = ?, birth_date = ?, birth_place = ?, gender = ?, civil_status = ?, contact_number = ?, email = ?, address3 = ?, resident_since = ? WHERE id = ?";

			if($stmt = $this->conn->prepare($query)) {
				$verify = 1;

				$stmt->bind_param('sssssssssi', $occupation, $birth_date, $birth_place, $gender, $civil_status, $contact_number, $email , $address3, $resident_since, $id);
				$stmt->execute();
				$stmt->close();
			}
		}
		...
		public function displayResidentsProfileBlotter($id) {
			$data = null;
			$query = "SELECT * FROM blotters WHERE resident_id = ? AND status = 1";
			if ($stmt = $this->conn->prepare($query)) {
				$stmt->bind_param('i', $id);
				$stmt->execute();
				$result = $stmt->get_result();
		...
		public function residentBlotterChecker($id, $blot_status) {
			$data = null;
			$query = "SELECT * FROM blotters WHERE resident_id = ? AND NOT blotter_status = ?";
			if ($stmt = $this->conn->prepare($query)) {
				$stmt->bind_param('is', $id, $blot_status);
				$stmt->execute();
				$result = $stmt->get_result();
				$num_of_rows = $stmt->num_rows;
				while ($row = $result->fetch_assoc()) {
		...
		public function residentBlotterCount($id, $status) {
		    $count = 0;
		    $query = "SELECT COUNT(*) as count FROM blotters WHERE resident_id = ? AND status = ?";
		    if ($stmt = $this->conn->prepare($query)) {
		...
		public function displayBlotterDetails($id) {
			$data = null;
			$query = "SELECT a.*, b.*, b.id as blotter_id FROM residents AS a INNER JOIN blotters AS b ON a.id = b.resident_id WHERE b.id = ?";
			if ($stmt = $this->conn->prepare($query)) {
				$stmt->bind_param('i', $id,);
				$stmt->execute();
				$result = $stmt->get_result();
		...
		public function changeResidentStatusX($id, $status ) {
        ...
        public function fetchPunongBrgy($status) {
			$data = null;
			$query = "SELECT * FROM org_structure WHERE status = ? AND position = '999' ORDER BY position ASC";
			if ($stmt = $this->conn->prepare($query)) {
				$stmt->bind_param('i', $status);
				$stmt->execute();
				$result = $stmt->get_result();
		...
		public function fetchOrgStructure($status) {
			$data = null;
			$query = "SELECT * FROM org_structure WHERE status = ? AND NOT position = '999' ORDER BY position ASC";
			if ($stmt = $this->conn->prepare($query)) {
				$stmt->bind_param('i', $status);
				$stmt->execute();
				$result = $stmt->get_result();
		...
		public function fetchRequestsHistory($status) {
			...
			$query = "SELECT a.*, a.status as request_status, b.prof_pic, b.birth_place, b.birth_date, b.fname, b.mname, b.lname, b.address, b.address2, b.address3, b.resident_since, b.civil_status, b.id AS resident_id FROM request AS a INNER JOIN residents AS b ON a.resident_id = b.id WHERE NOT a.status = '$status' ORDER BY a.id DESC";
			if ($stmt = $this->conn->prepare($query)) {
				$stmt->execute();
				$result = $stmt->get_result();
				$num_of_rows = $stmt->num_rows;
		...
		public function fetchMarriedResidents($status, $gender) {
		    $data = null;
		    
			$query = "SELECT * FROM residents WHERE status = ? AND gender = ? AND civil_status = 'Married' AND married = '0' ORDER BY lname ASC";
			if ($stmt = $this->conn->prepare($query)) {
			    $status = 1;
		...
		public function addMarriage($husband_id, $wife_id, $date_added) {
			$query = "INSERT INTO marriage (husband_id, wife_id, date_added) VALUES (?, ?, ?)";
			if ($stmt = $this->conn->prepare($query)) {
			    $stmt->bind_param('iis', $husband_id, $wife_id, $date_added);
			    $stmt->execute();
			    $stmt->close();
				return true;
			}
		...
		public function changeMarried($married, $id) {
			$query = "UPDATE residents SET married = ? WHERE id = ?";
			
			if($stmt = $this->conn->prepare($query)) {
				$stmt->bind_param('ii', $married, $id);
				$stmt->execute();
				$stmt->close();
			}
		}

		public function displayMarriedResidents() {
			$data = null;
		...
		public function deleteMarriage($id) {
			$query = "DELETE FROM marriage WHERE id = ?";
			if ($stmt = $this->conn->prepare($query)) {
			    $stmt->bind_param('i', $id);
			    $stmt->execute();
			    $stmt->close();
